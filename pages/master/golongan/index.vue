<template>
  <div>
    ...
    >
      ...
      <v-label class="my-2 font-weight-medium">Pangkat</v-label>
      <CustomBaseInput
        type="input"
...
const pages = ref({ title: "Golongan" });
...
const dialogTitle = ref("Tambah Golongan");
...
function addItem() {
  ...
  editedItem.value = {};
  dialogTitle.value = "Tambah Golongan";
  editedItem.value = {};
  dialog.value = true;
}
  ...
  await golonganService()
    ...
    .then((res: any) => {
      if (res.data.id) {
        ...
        dialogTitle.value = "Edit Golongan";
        dialog.value = true;
      }
    });
}
