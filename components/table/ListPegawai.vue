<template>
  <v-card>
    <div class="d-sm-flex align-center justify-space-between bg-primary">
      ...
      <v-btn variant="text" icon @click="handleRefreshItems" class="mr-2">
      ...
      <v-row class="justify-end">
        <v-col cols="12" md="3">
          <CustomBaseInput
            ...
            @update:model-value="handleApplyFilterInstansi"
            @click:clear="
              () => {
                filter.instansiId = null;
                handleApplyFilter();
              }
            "
            :items="listInstansi"
...
export default {
  ...
  methods: {
    ...
    handleApplyFilterInstansi() {
      ...
      this.$emit("change-instansi", filter.instansiId);
      this.$router.replace({
        path: this.$route.path,
        query: this.filter,
      });
